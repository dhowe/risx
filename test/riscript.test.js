...
describe('RiScript.v3', function () {
  ...
  const TRACE = { trace: 1 },
    LTR = 0,
    T = TRACE,
    PL = { preserveLookups: 1 },
    TRX = { trace: 1, traceTx: 1 },
    TPL = { preserveLookups: 1, trace: 1 };
  ...
  describe('Sequences', function () {
    it('Should support norepeat choice transforms', function () {
      ...
      for (let i = 0; i < count; i++) {
        let res = riscript.evaluate('$names=[a|b]\n$names $names.norepeat()');
    ...
    it('Should support norepeat symbol transforms', function () {
      ...
      let count = 10;
      for (let i = 0; i < count; i++) {
        let res = riscript.evaluate('$rule=[a|b|c|d|e]\n$rule.nr $rule.nr');
        // console.log(i,res);
        expect(/^[a-e] [a-e]$/.test(res)).true;
        let parts = res.split(' ');
        expect(parts.length).eq(2);
  ...
  describe('Gates', function () {
    ...
    it('Should extract operands from gate', function () {
      let json = { a: 3, $or: [{ b: { $lt: 30 } }, { c: /^p*/ }] };
      let query = new RiScript.Query(riscript, json);
      let operands = query.operands(riscript, json);
  ...
  describe('Choice', function () {
    ...
    it('Should resolve choices in context', function () {
      let res = riscript.evaluate('$bar:$bar', { bar: '[man | boy]' });
      // console.log(res);
    ...
    it('Should select non-weighted choices evenly', function () {
      ...
      for (let i = 0; i < 1000; i++) {
        let res = riscript.evaluate('[quite|]');
    ...
    it('Should resolve multiword choices', function () {
      let silent = RiScript.RiTa.SILENCE_LTS;
    ...
    it('Should resolve weighted choices', function () {
      ...
      let result = { b: 0, a: 0 };
      for (let i = 0; i < 100; i++) {
        let ans = riscript.evaluate('[a | b (3)]');
        // console.log(i, ans);
  ...
  describe('Evaluation', function () {
    ...
    it('Should resolve simple statics', function () {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        let res = riscript.evaluate('{#foo=[a|b|c|d]}$foo $foo $foo', {});
        // console.log(i + ") " + res);
        let pts = res.split(' ');
  ...
  describe('Transforms', function () {
    ...
    it('Should preserve non-existent transforms', function () {
      let silent = RiScript.RiTa.SILENT;
    ...
    it('Should resolve functions on context props with transforms', function () {
      let s = '$player.name.toUpperCase().toLowerCase()';
      let gameState = { player: { name: 'Wing' } };
      let res = riscript.evaluate(s, gameState);
      expect(res).eq('wing');

      let ctx = { bar: { baz: 'result' } }; // property transform
      let rs = riscript.evaluate('$foo=$bar.baz\n$foo', ctx); // no parens
      expect(rs).eq('result');
    });
    ...
    it('Should resolve transforms ending with punc', function () {
      ...
      let dog = { name: 'spot', color: 'white', hair: { color: 'white' } };
      expect(riscript.evaluate('It was $dog.hair.color.', { dog: dog })).eq(
        'It was white.'
      );
      expect(
        riscript.evaluate('It was $dog.color.toUpperCase()!', { dog: dog })
      ).eq('It was WHITE!');
    ...
    it('Should resolve property transforms in context', function () {
      let ctx = { bar: { result: 'result' } }; // property transform
      let rs = riscript.evaluate('$foo=$bar.result\n$foo', ctx); // no parens
      expect(rs).eq('result');
    });
    ...
    it('Should handle nested context', function () {
      ...
      let res = riscript.evaluate('#foo=$bar.color\n$foo', ctx);
      expect(res).eq('blue');
    });
  });
...
| falling like [the rain | a leaf]',
    ...
    it('Should handle longer grammars', function () {
      let res = RiGrammar.expand(rules);
      let lines = res.split(/\s*<br>\s*/g);
      // console.log(lines);
      expect(lines.length).eq(5);
      return;

      // TODO: syllables not correct here (not riscript issue)
      let syls = lines.map((l, i) => {
        let words = RiTa.syllables(l).split(' ');
        return words
          .map((w) => w.split('/'))
          .filter((w) => /\w/.test(w))
          .flat();
    ...
    it('Should resolve inline grammars', function () {
      ...
      let rs = riscript.evaluate(script);
    ...
    it('should call constructorJSON', function () {
      ...
      let gr1 = new RiGrammar(JSON.parse(json));
    ...
    it('should call expandFrom.weights.static', function () {
      ...
      rg.addRule('bird', 'hawk');
      rg.addRule('mammal', 'dog');
      let res = rg.expand({ start: 'mammal', trace: 0 });
    ...
    it('should call to/from JSON', function () {
      ...
      expect(function () {
        let gra = RiGrammar.fromJSON({ a: 'b' });
      }).to.throw();
      expect(function () {
        let gra = RiGrammar.fromJSON('non-JSON string');
      }).to.throw();
 ...
 bb',
  ...
  describe('Helpers', function () {
    ...
    it('#flattenTransforms', function () {
      let txs = [{ image: '.nr()' }, { image: '.uc' }, { image: '.art()' }];
