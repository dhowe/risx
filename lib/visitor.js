...
class BaseVisitor {
  constructor(riScript) {
    ...
    RiScript = this.scripting.constructor; // class hack
  }
  isCstNode(o) {
    return (
      typeof o === 'object' &&
...
class RiScriptVisitor extends BaseVisitor {
  ...
  script(ctx) {
    ...
    this.nolog =
      this.nolog ||
      typeof RiScript.RiTa === 'undefined' ||
      RiScript.RiTa.SILENT;
    let count = ctx.expr ? ctx.expr.length : 0;
    this.print(
      'script',
      "'" + RiScript.escapeText(this.input) + "' :: " + count + ' expression(s)'
    );
    if (!count) return '';
  ...
  gate(ctx) {
    ...
    try {
      mingoQuery = new RiScript.Query(this.scripting, raw);
    } catch (e) {
  ...
  assign(ctx, opts) {
    ...
    if (isStatic) {
      ...
      info = `${sym} = ${RiScript.escapeText(value)} [#static] ${
        opts?.silent ? '{silent}' : ''
      }`;
    } else {
      let $ = this;
  ...
  text(ctx) {
    ...
    this.print('text', RiScript.escapeText("'" + image + "'"));
    return image;
  }

  entity(ctx) {
    return this.nodeText;
    // let res = decode(this.nodeText);
    // this.print('entity', this.nodeText + ' -> ' + res);
    // return res;
  }
  ...
  choice(ctx, opts) {
    ...
    let original = this.nodeText,
      info = original;
    let choiceKey = RiScript.stringHash(original + ' #' + this.choiceId(ctx));
  ...
  hasNoRepeat(tfs) {
    let transforms = RiScript.transformNames(tfs);
  ...
  choose(options, excludes = []) {
    ...
    let index = RiScript.RiTa // use RiTa for randomSeed
      ? RiScript.RiTa.randi(valid.length)
  ...
  applyTransform(target, transform) {
    ...
    else if (typeof RiScript.transforms[tx] === 'function') {
      result = RiScript.transforms[tx](target);
    }
